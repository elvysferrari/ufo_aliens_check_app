...
class OptionsImageScreen extends StatefulWidget {
  final ImageModel image;
  OptionsImageScreen({super.key, required this.image });

  @override
  State<OptionsImageScreen> createState() => _OptionsImageScreenState();
}

class _OptionsImageScreenState extends State<OptionsImageScreen> {
  ...
  void _getLiked() async {
    var liked = await appController.getUserPostLike(widget.image.id);
    if(liked) {
     setState(() {
  ...
  void _addLike(bool value) async {
    await appController.addImageUserLike(widget.image.id, value);
    setState(() {
      visibleButtonLike = false;
    });
  }
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Text(widget.image.totalReal, style: const TextStyle(color: Colors.green, fontWeight: FontWeight.bold, fontSize: 14),),
                                    const SizedBox(width: 32,),
                                    const Text("FAKE: ", style: TextStyle(color: Colors.white60, fontWeight: FontWeight.bold),),
                                    Text(widget.image.totalFake, style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: 14),),
                                  ],
                                ),
                              ],
                            )
                            ...
                            SizedBox(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(visibleFullDescription ? "${widget.image.userName}: ${widget.image.fullDescription}" : "${widget.image.userName}: ${widget.image.shortDescription}", textAlign: TextAlign.start, style: const TextStyle(
